...
public class StripePaymentGateway implements PaymentGateway {
    ...
    public CheckoutSession createCheckoutSession(Order order) {
        try {
            ...
            System.out.println(ex.getMessage()); // should use logging service to log instead (e.g. sentry)
    ...
    public Optional<PaymentResult> parseWebhookRequest(WebhookRequest request) {
        try {
            // take WebhookRequest -> { orderId, paymentStatus } (PaymentResult)
            var payload = request.getPayload();
            var signature = request.getHeaders().get("stripe-signature");
            // Spring normalize the header's key when we inject all headers into a Map
